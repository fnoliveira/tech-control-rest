	...
	public ResponseEntity<Response<List<Avisos>>> retrieveAvisos(@PathVariable("condominioId") Long condominioId) {
		...
		if (avisos == null) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND)
					.body(new Response<List<Avisos>>(new ApiStatusResponse(HttpStatus.NOT_FOUND.value(), 4041,
							"Avisos Condominio with ID " + condominioId + " not found")));
		}

		return ResponseEntity
				.ok(new Response<List<Avisos>>(avisos, new ApiStatusResponse(HttpStatus.OK.value(), 2001, "Sucesso")));
	}

	@PostMapping(value = "/condominio/{condominioId}")
	public ResponseEntity<Response<Avisos>> saveAvisos(@PathVariable("condominioId") Long condominioId,
		...
		try {
			...
			return ResponseEntity
					.ok(new Response<Avisos>(aviso, new ApiStatusResponse(HttpStatus.OK.value(), 2001, "Sucesso")));
		} catch (Exception e) {
			e.printStackTrace();
